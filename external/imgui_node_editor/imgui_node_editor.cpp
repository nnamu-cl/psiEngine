...
// ImRect::Floor() was removed in newer ImGui versions - compatibility shim
static void ImRectFloor(ImRect& r) { r.Min = ImFloor(r.Min); r.Max = ImFloor(r.Max); }
...
void ed::EditorContext::SetGroupSize(NodeId nodeId, const ImVec2& size)
{
    ...
    if (node->m_GroupBounds.GetSize() != size)
    {
        ...
        ImRectFloor(node->m_GroupBounds);
...
void ed::NodeBuilder::PinRect(const ImVec2& a, const ImVec2& b)
{
    ...
    ImRectFloor(m_CurrentPin->m_Bounds);
