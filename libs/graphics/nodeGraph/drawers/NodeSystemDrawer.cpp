...
void NodeSystemDrawer::DrawInlineEditor(InputSocket *socket) {
    if (socket->isConnected()) return;

    ImGui::PushID(socket);
    switch (socket->editMode) {
        case SocketEditMode::Float:
            ImGui::PushItemWidth(80.0f);
            ImGui::DragFloat("##v", &std::get<float>(socket->defaultValue), 0.01f);
            ImGui::PopItemWidth();
            break;
        case SocketEditMode::Int:
            ImGui::PushItemWidth(80.0f);
            ImGui::DragInt("##v", &std::get<int>(socket->defaultValue));
            ImGui::PopItemWidth();
            break;
        case SocketEditMode::Vec2:
            ImGui::PushItemWidth(160.0f);
            ImGui::DragFloat2("##v", &std::get<glm::vec2>(socket->defaultValue)[0], 0.01f);
            ImGui::PopItemWidth();
            break;
        case SocketEditMode::Vec3:
            ImGui::PushItemWidth(220.0f);
            ImGui::DragFloat3("##v", &std::get<glm::vec3>(socket->defaultValue)[0], 0.01f);
            ImGui::PopItemWidth();
            break;
        case SocketEditMode::Vec4:
            ImGui::PushItemWidth(280.0f);
            ImGui::DragFloat4("##v", &std::get<glm::vec4>(socket->defaultValue)[0], 0.01f);
            ImGui::PopItemWidth();
            break;
        case SocketEditMode::None:
        default:
            break;
    }
    ImGui::PopID();
...
void NodeSystemDrawer::DrawInputSockets(Node *node) {
    for (InputSocket &input: node->getInputs()) {
...
void NodeSystemDrawer::DrawLinks() {
    ...
    for (const std::unique_ptr<Node> &node: m_NodeGraph->getNodes()) {
        ...
        for (const InputSocket &input: node->getInputs()) {
            if (input.connectedOutput) {
                ...
                // Gradient: from the output socket color to the input socket color
                ImU32 fromColor = ImColor(GetIconColor(input.connectedOutput->type));
                ImU32 toColor   = ImColor(GetIconColor(input.type));

                m_Connections.push_back({inputPinId, outputPinId, fromColor, toColor});
            }
        }
    }
    ...
    for (size_t i = 0; i < m_Connections.size(); ++i) {
        ...
        ed::Link(100000 + i, conn.outputPinId, conn.inputPinId, ImVec4(1,1,1,1), 1.5f, conn.fromColor, conn.toColor);
    }
}
